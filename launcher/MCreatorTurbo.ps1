...
$RegenJar    = Resolve-Payload 'regen-parallel-agent.jar'
...
function Load-Settings {
    ...
    if (Test-Path $SettingsPath) {
        try {
            ...
            foreach ($p in 'EditorRamGB','GradleRamGB','CpuCores','FastSave','ParallelRegen','Defender') {
...
function Build-JavaOptions([int]$editorRam, [bool]$fastSave, [bool]$parallelRegen, [bool]$regenAllowed) {
    $a = ""
    if ($fastSave -and (Test-Path $FastSaveJar)) { $a += "-javaagent:$FastSaveJar " }
    if ($parallelRegen -and $regenAllowed -and (Test-Path $RegenJar)) { $a += "-javaagent:$RegenJar " }
    return "$a-Xmx${editorRam}G"
}

function Apply-Defender([scriptblock]$log) {
    $paths = @("$env:USERPROFILE\.mcreator", "$env:USERPROFILE\MCreatorWorkspaces")
    $instRoot = $null
    if ($script:McExe) { $instRoot = Split-Path $script:McExe; $paths += $instRoot }
    $procs = @()
    if ($instRoot) {
        $procs += (Join-Path $instRoot 'jdk\bin\java.exe')
        $procs += (Join-Path $instRoot 'jdk\bin\javaw.exe')
        $procs += $script:McExe
    }
    try {
        $cur = (Get-MpPreference -ErrorAction Stop).ExclusionPath
        $missing = @($paths | Where-Object { $cur -notcontains $_ })
        if ($missing.Count -eq 0) { & $log "Defender exclusions already set."; return }
        $pl = ($paths | ForEach-Object { "'" + $_ + "'" }) -join ','
        $pr = ($procs | ForEach-Object { "'" + $_ + "'" }) -join ','
        $cmd = "Add-MpPreference -ExclusionPath $pl; Add-MpPreference -ExclusionProcess $pr"
        Start-Process powershell -Verb RunAs -Wait -WindowStyle Hidden -ArgumentList '-NoProfile','-Command',$cmd
        & $log "Defender exclusions applied (via one-time UAC prompt)."
    } catch {
        & $log "Skipped Defender step ($($_.Exception.Message))."
    }
}
...
$RegenAllowed   = ($script:Version -eq $PINNED_VERSION)
...
$fastChk.Text = "Fast-save agent (element saves skip the workspace-wide regen)"
...
$regenChk = New-Object System.Windows.Forms.CheckBox
$regenChk.Text = "Parallel-regen agent (Regenerate code uses all cores)"
$regenChk.Location = New-Object System.Drawing.Point(20, 356)
$regenChk.Size = New-Object System.Drawing.Size(510, 22)
$form.Controls.Add($regenChk)

$regenNote = New-Object System.Windows.Forms.Label
$regenNote.Location = New-Object System.Drawing.Point(40, 378)
$regenNote.Size = New-Object System.Drawing.Size(490, 20)
$regenNote.ForeColor = [System.Drawing.Color]::DimGray
$form.Controls.Add($regenNote)

function Refresh-RegenUI {
    $script:RegenAllowed = ($script:Version -eq $PINNED_VERSION) -and (Test-Path $RegenJar)
    if ($script:Version -eq $PINNED_VERSION) {
        $regenChk.Enabled = (Test-Path $RegenJar)
        $regenChk.Checked = [bool]$S.ParallelRegen -and $regenChk.Enabled
        $regenNote.Text = "Pinned build matches your MCreator ($PINNED_VERSION). Ready."
    } else {
        $regenChk.Enabled = $false
        $regenChk.Checked = $false
        $yours = if ($script:Version) { $script:Version } else { 'unknown' }
        $regenNote.Text = "Parallel regen is built for $PINNED_VERSION; yours is $yours, so it stays off."
    }
}
Refresh-RegenUI

$defChk = New-Object System.Windows.Forms.CheckBox
$defChk.Text = "Windows Defender exclusions (recommended; asks for admin once)"
$defChk.Location = New-Object System.Drawing.Point(20, 432)
$defChk.Size = New-Object System.Drawing.Size(510, 22)
$defChk.Checked = [bool]$S.Defender
$form.Controls.Add($defChk)
...
function Collect-Settings {
    return [pscustomobject]@{
        ...
        ParallelRegen = [bool]$regenChk.Checked
        Defender      = [bool]$defChk.Checked
    }
}
...
$launchBtn.Add_Click({
    ...
    if ($s.Defender) { Apply-Defender $log }
    $jo = Build-JavaOptions $s.EditorRamGB $s.FastSave $s.ParallelRegen $script:RegenAllowed
    ...
    try {
        ...
        & $log "Launched MCreator. Give it 15-30s to appear (longer for your big workspace)."
    } catch {
